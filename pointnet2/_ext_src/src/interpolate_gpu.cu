...
__global__ void
three_nn_kernel(int b, int n, int m, const scalar_t *__restrict__ unknown,
                const scalar_t *__restrict__ known,
                scalar_t *__restrict__ dist2, int *__restrict__ idx) {
  ...
  for (int j = index; j < n; j += stride) {
    ...
    for (int k = 0; k < m; ++k) {
      ...
      double d = static_cast<double>((ux - x) * (ux - x) + (uy - y) * (uy - y) +
                                     (uz - z) * (uz - z));
      if (d < best1) {
        best3 = best2;
        besti3 = besti2;
    ...
    dist2[j * 3 + 0] = static_cast<scalar_t>(best1);
    dist2[j * 3 + 1] = static_cast<scalar_t>(best2);
    dist2[j * 3 + 2] = static_cast<scalar_t>(best3);
...
void three_nn_kernel_wrapper(int b, int n, int m, const float *unknown,
  ...
  three_nn_kernel<float>
      <<<b, opt_n_threads(n), 0, stream>>>(b, n, m, unknown, known, dist2, idx);
...
void three_interpolate_kernel_wrapper(int b, int c, int m, int n,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  three_interpolate_kernel<float><<<b, opt_block_config(n, c), 0, stream>>>(
...
void three_interpolate_kernel_wrapper_bf16(int b, int c, int m, int n,
                                           ...
                                           at::BFloat16 *out) {
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  three_interpolate_kernel<at::BFloat16>
      <<<b, opt_block_config(n, c), 0, stream>>>(b, c, m, n, points, idx,
                                                 weight, out);
...
void three_interpolate_grad_kernel_wrapper(int b, int c, int n, int m,
                                           ...
                                           float *grad_points) {
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  three_interpolate_grad_kernel<float>
      <<<b, opt_block_config(n, c), 0, stream>>>(b, c, n, m, grad_out, idx,
                                                 weight, grad_points);
...
void three_interpolate_grad_kernel_wrapper_bf16(int b, int c, int n, int m,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  three_interpolate_grad_kernel<at::BFloat16>
      <<<b, opt_block_config(n, c), 0, stream>>>(b, c, n, m, grad_out, idx,
                                                 weight, grad_points);
